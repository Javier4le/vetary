...
import { createTestingApp, resetDb, seedClinic } from "../utils/test-helper";
...
describe("E2E — Cross-Tenant Isolation", () => {
	...
	async function login(email: string, password: string, tenantId: string): Promise<string> {
	...
	it("❌ Clinic A token rejected on Clinic B subdomain (GET /users)", async () => {
		...
		await seedClinic("Clínica B", "clinica-b", "admin@clinica-b.com", passwordHash, "ADMIN");
