...
describe("TenantGuard (unit)", () => {
	...
	function createContext(tenantId: string | null, userTenantId: string | null): ExecutionContext {
		return {
			switchToHttp: () => ({
				getRequest: () => ({
					tenant: tenantId
						? { id: tenantId, name: "Clinic", subdomain: "clinic", status: "ACTIVE" }
						: undefined,
					user: userTenantId
						? { userId: "u1", tenantId: userTenantId, role: "ADMIN", email: "a@b.com" }
						: undefined,
				}),
			}),
			getHandler: jest.fn(),
	...
	it("❌ Missing req.tenant or req.user → throws 403 'Tenant or user context missing'", () => {
		...
		expect(() => guard.canActivate(createContext(null, "tenant-1"))).toThrow(ForbiddenException);
		expect(() => guard.canActivate(createContext(null, "tenant-1"))).toThrow(
			"Tenant or user context missing",
		);

		expect(() => guard.canActivate(createContext("tenant-1", null))).toThrow(ForbiddenException);
